...
if __name__=="__main__":
    ...
    args.model_file_name = "Models_train_PPO"
    args.graph_pre_model = 683

    val_note = "_new_ppo_pretrain_zero_punish_val_"+str(args.graph_pre_model)
    ...
    # experiment_details = "graph_object_goal_navigation_adjacent_GAT_2025_05_26_23_38_39_multi_check_ppo"
    # experiment_details = "graph_object_goal_navigation_adjacent_GAT_2025_05_26_11_52_28_multi_check_ppo"
    # experiment_details = "graph_object_goal_navigation_adjacent_GAT_2025_06_04_14_32_52_multi_check_ppo_zero_punish_reward"
    experiment_details = "graph_object_goal_navigation_adjacent_GAT_2025_06_15_17_05_30_multi_check_ppo_zero_punish_reward"
    ...
    for index_in_episodes in tqdm(range(args.graph_episode_num)):   
        ...
        while True:
            ...
            if(int(np.sum(rl_graph.data['state']['action_mask'].cpu().numpy()))>0):
                polict_action, policy_acton_idx, action_logprob = policy.select_action(writer, rl_graph.data['state'], if_train=args.graph_train) # 1
                
                # world_cx, world_cy, world_cz, world_turn = get_current_world_pos(habitat_env)
                # action_node = policy.greedy_select_action_frontier_score(rl_graph, world_cx, world_cy)
                ...
                if(int(np.sum(rl_graph.data['state']['action_mask'].cpu().numpy()))>0) and (ghost_patch_res=="ok"):
                    print("=====> ghost_patch <=====")
                    polict_action, policy_acton_idx, action_logprob = policy.select_action(writer, rl_graph.data['state'], if_train=args.graph_train) # 2

...
def request_llm(rgb_image_ls, object_text, image_index_ls):
    ...
    input_ids = torch.cat(tuple(input_id_room_1_ls), dim=0)
    ...
    output_room_str_ls = outputs_all_str_ls[0:len(new_rgb_image_ls)]
    ...
    # try:
    #     room_type_score = np.max(output_room_score_ls)
    # except:
    #     room_type_score = 0
    # if(room_type_score<0.6):
    #     room_type_score = 0.0
    # # ===========> temp_close <===========
    # try:
    #     object_details_score = np.max(output_object_score_ls)
    # except:
    ...
    res_room_score_ls = [0, 0, 0, 0]
    for temp_index, image_index in enumerate(image_index_ls):
        try:
            temp_score = output_room_score_ls[temp_index]
            temp_score += 1
            temp_score -= 1
        except:
            temp_score = 0
        res_room_score_ls[image_index] = temp_score
    print("res_room_score_ls:", res_room_score_ls)
    print("image_index_ls:", image_index_ls)

    answer_ls = [res_room_score_ls]
    return answer_ls
...
# print(request_llm(rgb_image_ls, object_text="bed", image_index_ls=[0]))
# end_time = time.time()

...
from perception.tools import fix_depth, get_rgb_image_ls, get_gt_image_ls, get_large_depth
from perception.intention_utils_rcnn import object_detect
# from perception.intention_utils_groundsam import object_detect_grounding_sam
...
from perception.laser_utils import get_laser_point, get_laser_point_for_ring
...
class GraphMap(object):
    ...
    def add_ghost(self, final_frontier_pos_arr):
        for index in range(len(final_frontier_pos_arr)):
            ...
            rgb_image_ls = get_rgb_image_ls(self.habitat_env)
            new_frontier = Node(node_type="frontier_node", rela_cx=final_frontier_pos_arr[index][0], rela_cy=final_frontier_pos_arr[index][1], parent_node=self.current_node, world_cx=res_loc_in_real_world[0], world_cy=res_loc_in_real_world[1], rgb_image_ls=rgb_image_ls) 
    ...
    def add_intention(self, detect_res_pos_dict):
        ...
        for temp_score in detect_res_pos_dict:
            for temp_rela_pos in detect_res_pos_dict[temp_score]:
                ...
                res_col_index_factor = temp_rela_pos[3]
                ...
                new_intention = Node(node_type="intention_node", rela_cx=center_loc_in_ref[0], rela_cy=center_loc_in_ref[1], parent_node=self.current_node, score=temp_score, world_cx=res_loc_in_real_world[0], world_cy=res_loc_in_real_world[1], res_col_index_factor=res_col_index_factor)
                # correct_recheck
        ...
        for temp_intention_node in self.intention_nodes:
            ...
            if(min_dis<=1.0): # intention_node聚类距离修改
                ...
                if(min_node.robot_intention_dis<temp_intention_node.init_dis):
                    ...
                    temp_intention_node.near_dis_ls.append(min_node.robot_intention_dis)
                    temp_intention_node.near_res_col_index_factor_ls.append(min_node.res_col_index_factor)
        # correct_recheck
    

    '''
    def add_intention_gt(self, detect_res_pos_dict, rgb_image_ls, object_text):
        rela_loc = np.array([self.rela_cx, self.rela_cy])
        r_matrix = np.array([[np.cos(self.rela_turn), np.sin(self.rela_turn)], [-np.sin(self.rela_turn), np.cos(self.rela_turn)]])
        ...
        if(len(self.frontier_nodes)>0):
            ...
            for i in range(12):
                ...
                if not habitat_env.episode_over:
                    ...
                    # detect_res_pos_dict = object_detect_grounding_sam(rgb_image_ls, depth, object_goal)
                    # self.add_intention(detect_res_pos_dict)
                    
                    # detect_res_pos_dict = object_detect_gt(gt_image_ls, depth, object_goal, HabitatAction.object_id_num_ls)
                    # self.add_intention_gt(detect_res_pos_dict, rgb_image_ls, object_goal)

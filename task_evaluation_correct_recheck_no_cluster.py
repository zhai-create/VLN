...
if __name__=="__main__":
    ...
    if(args.is_llm==2):
        ...
        val_note = "_two_dim_long_time_series_val_"+str(args.graph_pre_model)+"_init_800"
    ...
    for index_in_episodes in tqdm(range(args.graph_episode_num)):   
        ...
        if(args.is_auto==False):
            ...
            cv2.imshow("depth", (observations["depth"]*255).astype(np.uint8))
            ...
            for temp_ghost in topo_graph.frontier_nodes:
                temp_ghost_rela_label_loc = np.array([temp_ghost.rela_cx, temp_ghost.rela_cy])
                t2 = temp_ghost_rela_label_loc/graph_args.resolution
                p2 = np.array([-t2[0], t2[1]])
                end_point = p2+np.array([100, 100])
                cv2.circle(occu_for_show, (int(end_point[1]), int(end_point[0])), 1, (0, 0, 0), -1)
